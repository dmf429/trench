...
interface Room {
  ...
  kolSentiment: { total: number; holding: number; holdingPercent: number }
  createdAt: string
}
...
export default function HomePage() {
  ...
  const loadRooms = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`${API}/rooms?sort=${sort}&search=${search}&limit=20`)
    ...
    <div style={{ minHeight: '100vh', background: '#050508', color: '#e0e0f0', fontFamily: "'DM Sans', sans-serif", paddingBottom: '48px' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: '52px', background: 'rgba(5,5,8,0.95)', borderBottom: '1px solid #1a1a2e', backdropFilter: 'blur(12px)' }}>
        ...
        <div style={{ display: 'flex', gap: '32px' }}>
          {['rooms', 'waitlist'].map(t => (
            <button key={t} onClick={() => setTab(t as any)} style={{ background: 'none', border: 'none', fontFamily: "'Share Tech Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: tab === t ? '#00FF88' : '#6666aa', cursor: 'pointer', borderBottom: tab === t ? '1px solid #00FF88' : '1px solid transparent', paddingBottom: '2px' }}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '32px', background: 'rgba(10,10,16,0.95)', borderTop: '1px solid #1a1a2e', display: 'flex', alignItems: 'center', overflow: 'hidden', zIndex: 100 }}>
        <div style={{ display: 'flex', gap: '48px', animation: 'ticker 30s linear infinite', whiteSpace: 'nowrap', padding: '0 24px', fontFamily: "'Share Tech Mono', monospace", fontSize: '10px' }}>
          {rooms.slice(0, 8).map(r => (
            <span key={r.id} style={{ color: r.priceChange24h >= 0 ? '#00FF88' : '#FF3366', flexShrink: 0 }}>
              ${r.token.symbol} {r.priceChange24h >= 0 ? '▲' : '▼'} {Math.abs(r.priceChange24h).toFixed(1)}%
            </span>
          ))}
          {rooms.slice(0, 8).map(r => (
            <span key={r.id + '_dup'} style={{ color: r.priceChange24h >= 0 ? '#00FF88' : '#FF3366', flexShrink: 0 }}>
              ${r.token.symbol} {r.priceChange24h >= 0 ? '▲' : '▼'} {Math.abs(r.priceChange24h).toFixed(1)}%
            </span>
          ))}
        </div>
          ...
          <>
            ...
            <div style={{ textAlign: 'center', padding: '48px 24px 32px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginBottom: '32px', position: 'relative' }}>
                {[
                  { val: rooms.length || '...', label: 'Live Rooms' },
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', position: 'relative' }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <span style={{ position: 'absolute', left: '12px', fontFamily: "'Share Tech Mono', monospace", fontSize: '14px', color: '#6666aa', pointerEvents: 'none' }}>⌕</span>
                  <input
                ...
                {[['volume','🔥 HOT'],['new','⚡ NEW'],['health','💊 HEALTH']].map(([v,l]) => (
                  <button key={v} onClick={() => setSort(v)} style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', letterSpacing: '1px', padding: '10px 14px', background: sort === v ? 'rgba(0,255,136,0.1)' : '#0a0a10', border: `1px solid ${sort === v ? 'rgba(0,255,136,0.4)' : '#1a1a2e'}`, color: sort === v ? '#00FF88' : '#6666aa', cursor: 'pointer' }}>{l}</button>
                ))}
                <button onClick={loadRooms} style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', letterSpacing: '1px', padding: '10px 14px', background: '#0a0a10', border: '1px solid #1a1a2e', color: '#6666aa', cursor: 'pointer' }}>↻ REFRESH</button>
              </div>
            </div>
            ...
            <div style={{ padding: '0 24px', maxWidth: '1400px', margin: '0 auto' }}>
              {loading ? (
                <div style={{ textAlign: 'center', padding: '60px', fontFamily: "'Share Tech Mono', monospace", fontSize: '12px', color: '#3a3a5c', letterSpacing: '2px' }}>
                  // LOADING LIVE DATA...
                </div>
              ) : rooms.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px', fontFamily: "'Share Tech Mono', monospace", fontSize: '12px', color: '#3a3a5c', letterSpacing: '2px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '8px' }}>
                  {rooms.map(room => {
                    ...
                    const health = room.healthScore
                    return (
                      <div key={room.id} style={{ background: '#0a0a10', border: '1px solid #1a1a2e', padding: '16px', cursor: 'pointer', transition: 'all 0.15s', position: 'relative', overflow: 'hidden' }}
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '24px', color: getRisk(health), lineHeight: 1 }}>{health}</div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '10px' }}>
                          ...
                          <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '11px', color: up ? '#00FF88' : '#FF3366', fontWeight: 'bold' }}>
                            {up ? '▲' : '▼'} {Math.abs(room.priceChange24h).toFixed(1)}%
                          </span>
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '10px', borderTop: '1px solid #1a1a2e' }}>
                          ...
                          {room.flagCount > 0 && (
                            <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: '#FF3366' }}>⚠ {room.flagCount} flags</span>
                          )}
        ...
        {tab === 'waitlist' && (
          <div style={{ textAlign: 'center', padding: '80px 24px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '12px', color: '#6666aa', maxWidth: '440px', margin: '0 auto 40px', lineHeight: 1.8 }}>
                First 1,000 wallets get lifetime access to premium features and priority $TRENCH allocation.
              </div>
              ...
              <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: '#3a3a5c', letterSpacing: '2px' }}>
