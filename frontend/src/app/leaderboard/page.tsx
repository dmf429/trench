...
const MOCK_LEADERBOARD = [
  ...
  { rank: 2, address: 'DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKH', displayName: 'Murad', twitterHandle: 'MustStopMurad', pnl7d: 54000, pnl30d: 280000, winRate: 61, totalTrades: 223, badge: 'kol' },
  { rank: 3, address: 'FnXBBsZRFjBdYX6YmSQQYXnmL2yZWcTDrPwEFGDGpump', displayName: 'DegenSpartan', twitterHandle: 'DegenSpartanAI', pnl7d: 42000, pnl30d: 210000, winRate: 63, totalTrades: 891, badge: 'whale' },
  { rank: 4, address: '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', displayName: 'Cobie', twitterHandle: 'cobie', pnl7d: 31000, pnl30d: 190000, winRate: 72, totalTrades: 412, badge: 'top-caller' },
  { rank: 5, address: 'HVh6wHNBAsQDXVL42aCBB6R3UWrMBBV5sMfpbmZfDPQT', displayName: 'Hsaka', twitterHandle: 'HsakaTrades', pnl7d: 28000, pnl30d: 142000, winRate: 64, totalTrades: 634, badge: 'kol' },
...
export default function LeaderboardPage() {
  ...
  const sorted = [...MOCK_LEADERBOARD].sort((a, b) => period === '30d' ? b.pnl30d - a.pnl30d : b.pnl7d - a.pnl7d).map((k, i) => ({ ...k, rank: i + 1 }))
      ...
      <div style={{maxWidth:'1000px',margin:'0 auto',padding:'72px 24px 48px'}}>
          ...
          {podium.map((kol, i) => {
            ...
            return (
              <div key={kol.rank} style={{background:isFirst?'rgba(255,215,0,0.05)':'#0a0a10',border:`1px solid ${isFirst?'rgba(255,215,0,0.3)':'#1a1a2e'}`,padding:isFirst?'28px 20px':'20px',textAlign:'center',position:'relative',overflow:'hidden'}}>
                ...
                <div style={{width:isFirst?'48px':'40px',height:isFirst?'48px':'40px',borderRadius:'50%',background:`hsl(${kol.rank*40+120},60%,35%)`,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Bebas Neue',sans-serif",fontSize:isFirst?'20px':'16px',color:'#fff',margin:'0 auto 12px'}}>{kol.displayName[0]}</div>
                ...
                {kol.badge && BADGES[kol.badge] && (
                  <div style={{display:'inline-block',fontFamily:"'Share Tech Mono',monospace",fontSize:'8px',letterSpacing:'1px',padding:'3px 8px',background:BADGES[kol.badge].bg,color:BADGES[kol.badge].color,border:`1px solid ${BADGES[kol.badge].border}`,marginTop:'10px'}}>{BADGES[kol.badge].label}</div>
                )}
              </div>
            )
          })}
        ...
        <div style={{background:'#0a0a10',border:'1px solid #1a1a2e'}}>
          ...
          {sorted.map(kol => {
            ...
            const rankColor = RANK_COLORS[kol.rank] || '#6666aa'
            return (
              <div key={kol.rank} style={{display:'grid',gridTemplateColumns:'60px 1fr 120px 120px 80px 80px',padding:'14px 20px',borderBottom:'1px solid #0d0d18',transition:'background 0.15s'}}
              ...
              >
                ...
                <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                  ...
                  <div>
                    ...
                    <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'9px',color:'#6666aa'}}>@{kol.twitterHandle} · {tr(kol.address)}</div>
                  </div>
                </div>
        ...
        <div style={{marginTop:'24px',background:'rgba(0,255,136,0.03)',border:'1px solid rgba(0,255,136,0.15)',padding:'24px',textAlign:'center'}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'24px',letterSpacing:'4px',marginBottom:'8px'}}>WHERE DO YOU RANK?</div>
          ...
          <a href="/waitlist" style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'11px',letterSpacing:'2px',color:'#050508',background:'#00FF88',border:'none',padding:'12px 28px',cursor:'pointer',textDecoration:'none',clipPath:'polygon(10px 0%,100% 0%,calc(100% - 10px) 100%,0% 100%)',display:'inline-block'}}>
            JOIN WAITLIST →
          </a>
        </div>
      </div>
